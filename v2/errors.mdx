---
title: Errors
description: Error envelopes, status codes, tool validation errors, and how to handle them.
---


Most v2 errors use the envelope below. [Tool-definition validation](/v2/tools#validation-and-runtime-errors)
is an exception: it returns `{"error":"invalid_tools","details":[...]}` with
all offending field paths. Handle that shape when saving agent tools.
...
| `403` | The key is valid but **revoked**, or the call is not covered by your company's plan. | No. |
...
| `503` | Temporarily unavailable (deploy, capacity) — or a feature that is not provisioned for your workspace. | Yes for `fleet_offline`; no for `campaigns_unavailable`, which needs provisioning, not patience. |
...
| `forbidden` | `403` | The key was revoked. | Get a new key. |
| `contract_unavailable` | `403` | The call is not covered by an active production contract: `tier` is not your company's plan, or the company has not finished going live. Nothing was dialled. | Leave `tier` out — calls default to your plan's tier. See [setting the tier](/general/tiers#setting-the-tier). |
| `insufficient_balance` | `402` | Pre-dial wallet gate. No call was placed and nothing was charged. The same condition inside a running [campaign](/v2/campaigns) is not an error — the campaign pauses with `pause_reason: "insufficient_balance"`. | [Top up](/v2/wallet#top-up) and retry with the same `Idempotency-Key`. |
| `not_found` | `404` | Unknown `agt_`/`call_`/`cmp_` ID, a soft-deleted agent, a number not on the [do-not-call list](/v2/dnc), a transcript or recording that does not exist, or an ID that belongs to another workspace. | Check the ID. For artifacts, check `transcript_available` / `recording_available` on the call. |
...
| `at_capacity` | `429` | No capacity for this request: on calls, every line on the deployment is busy — a fleet-wide ceiling, not your workspace's. On [text to speech](/v2/tts-quickstart#limits) it also means your workspace already has its 10 requests in flight. Nothing was charged either way. | Honour `Retry-After` and retry. |
| `flow_agent_web_only` | `409` | A phone call or campaign named a node-based agent. Node-based agents run as browser calls for now. | Use a single-prompt agent for phone calls and campaigns. See [tiers](/general/tiers#one-stack-on-every-tier). |
| `tier_unavailable` | `501` | `tier` names a tier that is not enabled on this deployment. | Leave `tier` out — calls default to your plan's tier. See [tiers](/general/tiers#setting-the-tier). |
| `campaigns_unavailable` | `503` | Campaigns are not enabled for your workspace. Every `/v2/campaigns` and `/v2/dnc` route answers this; everything else keeps working. | Not a mistake in your request, and retrying will not clear it — ask your Mirai contact to enable them. |
| `fleet_offline` | `503` | Speech capacity is offline, so a [campaign start](/v2/campaigns#start-a-campaign) was refused. Nothing was charged and the campaign stayed where it was. | Honour `Retry-After` (300s) and start it again. |
...
An `agt_`, `call_` or `cmp_` ID that exists in another workspace is answered exactly as
...
<Tabs>
  ...
  <Tab title="Node.js">
    ...
    export async function request(path, init = {}, attempt = 0) {
      const res = await fetch(`https://sandbox.voice.miraiminds.co${path}`, {
        ...init,
        headers: {
...
- **Never retry** `400`, `401`, `403`, `404`, `501` — the same request will fail
  the same way. `503 campaigns_unavailable` belongs in this group too: it is a
  provisioning fact, not a wobble.
