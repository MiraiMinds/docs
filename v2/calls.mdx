...
| `tier` | string | The rate card this call was billed under — your company's plan. See [tiers](/general/tiers). |
...
| `transferred` | `completed` | The call ended in a completed warm transfer to a human. Handoff is [part of every tier](/general/tiers#one-stack-on-every-tier); the rollout is completing — **no call emits this yet**, see the [roadmap](/general/roadmap). |
...
rollout and later releases — see the [roadmap](/general/roadmap).
</Note>
...
| `tier` | string | no | `t1` \| `t3` \| `t5`. Defaults to your key's tier, which is your company's plan — leave it out. Any other tier is refused with `403 contract_unavailable`. See [tiers](/general/tiers#setting-the-tier). |
...
| `403` | `contract_unavailable` | The call is not covered by an active production contract: `tier` is not your company's plan, or the company has not finished going live. See [setting the tier](/general/tiers#setting-the-tier). |
...
| `409` | `flow_agent_web_only` | `agent_id` is a node-based agent. Node-based agents run as browser calls for now; phone calls for flows are coming. Use a single-prompt agent. |
...
| `501` | `tier_unavailable` | `tier` names a tier that is not enabled on this deployment |
...
- Plain `{{key}}` placeholders match loosely: letters and digits only,
  case-insensitive, so `{{Customer Name}}`, `{{customer_name}}` and
  `{{CustomerName}}` all read the same variable. Only a Liquid expression
  with a filter (`{{ key | upcase }}`) needs the exact key.
