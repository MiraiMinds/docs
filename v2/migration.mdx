...
- **[Campaigns](/v2/campaigns)** — a server-side dialler with calling windows,
  retries, budgets, do-not-call suppression and a per-contact report.

You can migrate function calling to [v2 tools](/v2/tools), including background
on-call requests and post-call actions. Recreate each tool definition on your
v2 agent; v1 tool IDs do not transfer. Review each remaining capability before
cutover, and keep your v1 integration active until its replacement is tested.

## Hosts

| | v1 | v2 |
| :--- | :--- | :--- |
| Base URL | `https://api.voice-agents.miraiminds.co` | `https://sandbox.voice.miraiminds.co` |
...
| `/v1/admin/tool/api` (all methods) | [`/v2/agents/{id}/tools/{name}`](/v2/tools#api-routes) | Tools belong to an agent; recreate the definition instead of copying a v1 ID. |
...
| — | [`GET /v2/calls/{id}/transcript`](/v2/calls#get-the-transcript) | **New.** Turn-by-turn, as spoken. |
| — | [`GET /v2/calls/{id}/recording`](/v2/calls#get-the-recording) | **New.** `302` to a 15-minute signed URL. v1's `recordingUrl` has a v2 home at last. |
| `callSettings.slots` + your own dialler | [`POST /v2/campaigns`](/v2/campaigns#create-a-campaign) and the rest of `/v2/campaigns/*` | **New.** The calling window, retries and pacing that lived in v1's assistant config are now a first-class resource. |
| — | [`POST /v2/dnc`](/v2/dnc#add-a-number) | **New.** Workspace suppression list, enforced at dial time. |
...
| `agent.tools` | [`tools`](/v2/tools) — full definitions, not v1 tool IDs |
| `telephony.inbound` / `telephony.outbound` | — (managed) |
| `callSettings.slots` (calling window) | `slots` + `timezone` on a [campaign](/v2/campaigns#calling-window) (or [your scheduler](/v2/limits#calling-window) for single calls) |
| `callSettings.concurrentCallCount` | `max_concurrent` on a [campaign](/v2/campaigns); the workspace ceiling is provisioned per key, see [Limits](/v2/limits) |
| `callSettings.retryProtocol` | `retry_count` + `re_attempt_period_secs` on a [campaign](/v2/campaigns#retries) |
| `analysisPlan` | — (post-call analysis is part of every tier; its v2 surface is on the [roadmap](/general/roadmap)) |
...
+POST https://sandbox.voice.miraiminds.co/v2/calls
...
| `timeout` | `timeout` |
| `aborted` | `aborted` |
| `skip` | contact status [`suppressed`](/v2/campaigns#contact-statuses) inside a campaign; nothing for single calls |
| `rescheduled` | contact status `pending` again, until [`retry_count`](/v2/campaigns#retries) runs out and it becomes `exhausted` |
| — | `voicemail` (**new**) |
...
| Event count | 15 types incl. `call.initiate`, `call.ended`, `end-of-call`, `action` | 6 call types + 5 [`campaign.*`](/v2/campaigns#webhooks) types |
...
- **`recordingUrl` is not on the event.** The audio is a separate, authenticated
  request — [`GET /v2/calls/{id}/recording`](/v2/calls#get-the-recording) — which
  redirects to a 15-minute signed URL. A permanent link in a webhook body is a
  recording anybody who ever saw that body can still fetch.
...
- **No `end-of-call` analysis event yet.** Post-call analytics (AI summary +
  QA score on every call) is [part of every tier](/general/tiers#one-stack-on-every-tier)
...
5. Recreate and test your [tools](/v2/tools) on the v2 agent, then migrate the
   remaining calls. Check support for other capabilities before moving them.
