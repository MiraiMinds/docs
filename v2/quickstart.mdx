...
Five minutes, four requests, one real phone call. Calls bill at your plan's
[tier](/general/tiers); the example call below ran on T3.
...
<Tip>
That prompt is the minimum that works. Before you run it at volume, read the
[prompting guide](/v2/prompting-guide) — the structure, the Hinglish rules, and
...
96 seconds is billed as 10 blocks of 10 seconds — ₹5.00 on T3.
[Blocks are whole and rounded up](/general/tiers#metering), with a one-block
minimum.
</Step>
</Steps>
...
- [Billing & tiers](/general/tiers) — the price ladder and what a minute costs.
...
| `402 insufficient_balance` | Wallet cannot cover one minute at your tier (₹5 on T5) | Check `GET /v2/wallet`; add credit from the console's **Billing** page, or ask your Mirai contact before you go live |
| `400 invalid_request` on `voice_id` | The voice is not in the catalogue | `ashu`, `neha`, `shruti` or `sameer`, on every tier; see [Voices](/v2/voices) |
