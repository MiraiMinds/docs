...
[`GET /v1/admin/voice-gallery`](/v1/assistants#voice-gallery), which returns a
slug and a sample audio URL per voice. See [Voices](/v2/voices) for the full
discovery flow.
...
The tier-1 product is a **transactional** call — one job, done in under two
minutes. Prompts that work:
...
that is a `t3`/`t5` feature, see [tiers](/general/tiers).
