...
| TTS requests in flight | 10 **per workspace** | `/v1/audio/speech` and `/v2/tts` | `429 at_capacity` |
...
**Call** concurrency does **not** produce a `429`. A `POST /v2/calls` beyond
...
call path, `429` is request-rate only. ([Text to speech](#text-to-speech) is the
...
The [speech API](/v2/tts-quickstart) has no queue, so its concurrency is a gate,
not a pacer.

| Limit | Default | Over the limit |
| :--- | :--- | :--- |
| Requests in flight | **10 per workspace** | `429 at_capacity` with `Retry-After`. The request is refused, not queued. |
| `input` length | **2,000 characters** | `400 invalid_request`. Split longer text. |
| Request rate | 10 requests/second, burst 20, per key | `429 rate_limited` with `Retry-After`. |

`at_capacity` can also come from a deployment-wide ceiling when the speech node
...
| TTS `input` | 2,000 characters per request |
...
stack is tuned for prompts in the hundreds of tokens, not thousands.
...
| TTS requests in flight | ✅ `429 at_capacity` past 10 per workspace |
