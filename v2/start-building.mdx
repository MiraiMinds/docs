...
**in 10-second blocks, rounded up, one-block minimum**, at your plan's
[tier](/general/tiers). The examples below ran on T3: ₹3 a minute, ₹0.50 a
block.
...
<Tip>
`voice_id` is `ashu`, `neha`, `shruti` or `sameer` — the same catalogue on
every tier. See [Voices](/v2/voices).
</Tip>
...
96 seconds rounds up to 10 blocks of 10 seconds — ₹5 on T3. See
...
Once your company is live, top up from the **Billing** page of the console.
</Accordion>
...
| `contract_unavailable` | `403` | The call's `tier` is not your plan's, or the company has not finished going live. Leave `tier` out. |
| `flow_agent_web_only` | `409` | Node-based agents run as browser calls for now. Use a single-prompt agent for phone calls and campaigns. |
...
<CardGroup cols={2}>
  ...
  <Card title="Billing & tiers" icon="indian-rupee-sign">
    The price ladder and what a minute costs. [Reference](/general/tiers).
  </Card>
</CardGroup>
